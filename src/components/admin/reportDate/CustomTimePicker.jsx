...
const CustomTimePicker = ({ title, btnInnerTime,setToState }) => {
    const [searchParams] = useSearchParams()
    const [hours, setHours] = useState((title == "שעת התחלה" ? searchParams?.get('startTime')?.substring(0, 2) :  searchParams?.get('endTime')?.substring(0, 2) || '')?.replace(":",""));
    const [minutes, setMinutes] = useState((title == "שעת התחלה" ? searchParams?.get('startTime')?.substring(2)  : searchParams?.get('endTime')?.substring(2) || '')?.replace(":",""));
    const minutesRef = useRef(null);

    useEffect(() => {
        setToState(`${hours?.padStart(2, '0')}:${minutes?.padStart(2, '0')}`)
    }, [hours,minutes])
                ...
                <div className="back relative">
                    ...
                    <div className="mt-4">
                        ...
                        <input
                                ...
                                placeholder="דקות"
                                className="ml-1 p-1 outline-none bg-transparent w-full border border-gray-300 rounded-r-lg"
                                min="0"
                                max="59"
                                maxLength="2"
                            />
                            <span className="p-1">:</span>
                              ...
                              <input
                                ...
                                placeholder="שעות"
                                className="mr-1 text-right p-1 outline-none bg-transparent w-full border border-gray-300 rounded-l-lg"
                                min="0"
                                max="23"
                                maxLength="2"
                            />
                        ...
                        <div className="my-4">
                            <p>
                                זמן שנבחר {hours?.padStart(2, '0')}:{minutes?.replace(/:/g, "")?.padStart(2, '0')}
                            </p>
                        </div>
