            ...
            <div className='  h-[45vh] overflow-y-auto mt-6 px-1 '>
                <div className="  grid grid-cols-2 gap-x-20 gap-y-7">
                    {SYSTEMSTRACT?.map((item, index) => (
                        <button onClick={() => navigation(`/startReport/${item?.value}?report=grup&users=${encodeURIComponent(JSON.stringify(usersName))}`)} key={index} className=" flex flex-col items-center justify-center gap-2">
