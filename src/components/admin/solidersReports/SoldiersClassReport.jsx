    ...
    return (
        <div dir='rtl' className='mt-7 w-full h-full flex flex-col flex-1'>
            ...
            <div className='  h-[45vh] overflow-y-auto mt-6 px-1 '>
                <div className="  grid grid-cols-2 gap-y-20 gap-x-7">
                    {SYSTEMSTRACT?.map((item, index) => (
                        <button onClick={() => navigation(`/startReport/${item?.value}?&access=manager&report=grup&users=${encodeURIComponent(JSON.stringify(usersName))}`)} key={index} className=" flex flex-col items-center justify-center gap-2">
