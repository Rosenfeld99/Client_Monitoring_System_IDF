...
const DisplayUser = ({ setUsersSelected, userDisplay, sampleUsers }) => {
    ...
    return (<>
        ...
        {userDisplay?.lastsReports.map((requsts, i) =>
            <>
                <div key={i} onClick={() => { !chooseReport && sampleUsers ? setUsersSelected(["has users"]) : setUsersSelected([]); setChooseReport(!chooseReport); }} className={` ${chooseReport && 'bg-slate-100 dark:bg-[#121212] border-[1px] border-[#62bcee] rounded-lg'} min-h-10 grid grid-cols-12 my-1 gap-3 py-1 `} >
